---
name: skill-reviewer
description: Reviews ZHarness skill packages for readiness, triggers, safety boundaries, resources, and evidence. Invoke when users ask to audit, grade, or production-check an existing skill.
allowed-tools:
...
2. Call `review_skill_package`.
   - Use `profile="zharness"` unless the user explicitly asks for portability against another skill spec.
