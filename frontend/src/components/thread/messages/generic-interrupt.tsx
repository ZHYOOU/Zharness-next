...
export function GenericInterruptView({
  ...
  // Truncate long strings while preserving URLs. / 截断长字符串，同时保留 URL。
  const truncateValue = (value: any): any => {
    if (typeof value === "string" && value.length > 100) {
      // Keep URLs complete and clickable. / 保持 URL 完整且可点击。
      if (isUrl(value)) {
        return value;
      }
    ...
    if (isComplexValue(value) && !isExpanded) {
      ...
      if (strValue.length > 100) {
        // Return plain text for truncated structured content. / 将截断的结构化内容返回为纯文本。
  ...
  // Process entries according to the expanded state. / 根据展开状态处理条目。
  const processEntries = () => {
    if (Array.isArray(interrupt)) {
      ...
      if (!isExpanded && shouldTruncate) {
        // Truncate eligible values while collapsed. / 折叠时截断符合条件的值。
