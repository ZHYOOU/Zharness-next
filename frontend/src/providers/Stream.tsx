...
const StreamSession = ({
  ...
  const streamValue = useTypedStream({
    ...
    onThreadId: (id) => {
      setThreadId(id);
      // Refetch after the thread ID changes, waiting for creation to finish. / thread ID 变化后重新获取列表，并等待新 thread 创建完成。
...
// Default values for the form. / 表单默认值。
...
export const StreamProvider: React.FC<{ children: ReactNode }> = ({
  ...
  // Use URL parameters with environment-variable fallbacks. / 优先使用 URL 参数，并以环境变量作为回退。
  ...
  // Store the API key locally and fall back to the environment. / 在本地存储 API 密钥，并以环境变量作为回退。
  ...
  // Show setup when the API URL or assistant ID is missing. / 缺少 API URL 或助手 ID 时显示设置表单。
        ...
        <div className="animate-in fade-in-0 zoom-in-95 bg-background flex max-w-3xl flex-col rounded-lg border shadow-lg">
          <div className="mt-14 flex flex-col gap-2 border-b p-6">
            <div className="flex flex-col items-start gap-2">
              ...
              <h1 className="text-xl font-semibold tracking-tight">ZHarness</h1>
            </div>
            <p className="text-muted-foreground">
              Welcome to ZHarness! Before you get started, you need to enter the
...
// Create a custom hook for the stream context. / 为流上下文创建自定义 hook。
