...
export function useFileUpload({
  ...
  // Register drag-and-drop handlers. / 注册拖放处理器。
  useEffect(() => {
    if (!dropRef.current) return;

    // Track nested global drag events with a counter. / 使用计数器跟踪嵌套的全局拖放事件。
    ...
    // Prevent the browser's global drag-over behavior. / 阻止浏览器默认的全局拖放悬停行为。
    ...
    // Keep element drag-over local while handling drops globally. / 保留元素级悬停处理，并在全局处理放置事件。
  ...
  /**
   * Handle pasted image and PDF files for text inputs.
   *
   * 处理文本输入框中粘贴的图片和 PDF 文件。
   */
