...
NEXT_PUBLIC_ASSISTANT_ID=lead_agent
...
# Add tags via the model configuration. / 通过模型配置添加标签。
...
# Prefix the ID before saving state. / 保存状态前添加 ID 前缀。
...
const streamValue = useTypedStream({
  ...
  // Configure other fields as needed. / 按需配置其他字段。
  defaultHeaders: {
    Authentication: `Bearer ${addYourTokenHere}`, // Pass the authentication token. / 传入身份验证令牌。
  },
});
```
